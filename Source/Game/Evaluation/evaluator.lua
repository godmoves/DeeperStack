...
--- Gives fast strength representations for all private hands on the given board.
-- @param board a possibly empty vector of board cards
-- @return a vector containing a strength value
function M:batch_eval_fast(board)
  ...
  elseif board:dim() == 1 then
    ...
    hands[{{}, {1, board:size(1)}}]:copy(board:view(1, board:size(1))
      :expand(game_settings.hand_count, board:size(1)))
